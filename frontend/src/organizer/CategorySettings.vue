...
const editor = reactive({ id: null, name: '', iconKey: 'ticket', displayOrder: 0, enabled: true })
let saving = false

function reset() { Object.assign(editor, { id: null, name: '', iconKey: 'ticket', displayOrder: 0, enabled: true }) }
function edit(item) { Object.assign(editor, { id: item.id, name: item.name, iconKey: item.iconKey || 'ticket', displayOrder: item.displayOrder, enabled: item.enabled }) }
async function save() {
  ...
  try {
    await props.api(editor.id ? `/api/organizer/catalog/categories/${editor.id}` : '/api/organizer/catalog/categories', {
      ...
      body: JSON.stringify({ name: editor.name.trim(), iconKey: editor.iconKey.trim(), displayOrder: Number(editor.displayOrder), enabled: editor.enabled }),
    })
  ...
  <div class="org-category-layout">
    <section class="org-category-list">
      ...
      <article v-for="item in categories" :key="item.id">
        ...
        <div><strong>{{ item.name }}</strong><small>{{ item.iconKey || 'ticket' }}</small></div>
