...
当前正式产品分为两个独立入口：购票用户从 EventRush 前台发现活动、完成交易并管理个人票务；活动主办方从 `/organizer/events` 管理活动发布。验票、运营和工程证据继续保留兼容入口，但不进入这两套日常导航。
...
座位图、真实支付和多角色登录暂不进入本阶段；主办方中心已在 Stage 53 建成，下一步只围绕真实运营场景补充能力，不把内部证据工具重新塞回主办方导航。
