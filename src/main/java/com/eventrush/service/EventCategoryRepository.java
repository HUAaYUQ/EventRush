...
public class EventCategoryRepository {
    ...
    private static final List<String[]> DEFAULT_CATEGORIES = List.of(
            new String[]{"演唱会", "mic", "PERFORMANCE"},
            new String[]{"话剧歌剧", "masks", "PERFORMANCE"},
            new String[]{"体育赛事", "trophy", "SPORTS"},
            new String[]{"展览休闲", "image", "EXHIBITION"},
            new String[]{"儿童亲子", "baby", "FAMILY"},
            new String[]{"音乐会", "music", "PERFORMANCE"},
            new String[]{"舞蹈芭蕾", "sparkles", "PERFORMANCE"},
            new String[]{"曲艺杂谈", "theater", "PERFORMANCE"},
            new String[]{"动漫展会", "gamepad", "EXHIBITION"}
    );
        ...
        for (int index = 0; index < DEFAULT_CATEGORIES.size(); index++) {
            ...
            jdbcTemplate.update("""
                    INSERT INTO event_category
                        (name, icon_key, content_profile, display_order, enabled, created_time, updated_time)
                    VALUES (?, ?, ?, ?, TRUE, ?, ?)
                    """, category[0], category[1], category[2], index * 10,
    ...
    public List<EventCategory> list(boolean enabledOnly) {
        ...
        return jdbcTemplate.query("""
                        SELECT id, name, icon_key, content_profile, display_order, enabled, created_time, updated_time
                        FROM event_category
    ...
    public EventCategory create(
        ...
        jdbcTemplate.update(connection -> {
            ...
            statement.setString(3, contentProfile);
    ...
    public EventCategory update(
        ...
        int updated = jdbcTemplate.update("""
                        ...
                        SET name = ?, icon_key = ?, content_profile = ?, display_order = ?, enabled = ?, updated_time = ?
                        WHERE id = ?
                        """, name, iconKey, contentProfile, displayOrder, enabled,
    ...
    private EventCategory map(java.sql.ResultSet resultSet) throws java.sql.SQLException {
        return new EventCategory(
                ...
                resultSet.getString("icon_key"), resultSet.getString("content_profile"),
